...
infer env ex = case ex of
  ...
  Lam (PLit lit) e -> do
    (s1, t1) <- infer env (Lit lit)
    (s2, t2) <- infer (apply s1 env) e
    return (s2 `compose` s1, t1 `TArrow` t2)
  Lam (PCons x xs) e -> do
    tv <- fresh                         

    let bindPat :: Pattern -> Type -> TypeEnv -> TypeEnv
        bindPat (PVar v) t env = env `extend` (v, Forall [] t)
        bindPat (PLit _) _ env = env     
        bindPat (PCons p1 p2) t env =     
            let env'  = bindPat p1 tv env           
                env'' = bindPat p2 (TArray tv) env' 
            in env''
    
    let env' = bindPat x tv env           -- x : tv
    let env'' = bindPat xs (TArray tv) env'  -- xs : [tv]
    
    (s1, t1) <- infer env'' e
    return (s1, TArray (apply s1 tv) `TArrow` t1)
  ...
  Op Cons e1 e2 -> do
    ...
    (s1, t1) <- infer env e1
    (s2, t2) <- infer (apply s1 env) e2
    s3 <- unify (apply s2 t2) (TArray (apply s2 t1))
  ...
  Op Concat e1 e2 -> do
    ...
    (s1, t1) <- infer env e1
    (s2, t2) <- infer (apply s1 env) e2
    s3 <- unify (apply s2 t1) (TArray tv)
    s4 <- unify (apply s3 t2) (TArray (apply s3 tv))
    return (s4 `compose` s3 `compose` s2 `compose` s1, TArray (apply s4 tv))
  ...
  Lit (LArray e) -> do
    tv <- fresh
    (s, _) <- foldM (\(s,_) x -> do
                (s1,t1) <- infer (apply s env) x
                s2 <- unify (apply (s1 `compose` s) tv) t1
                return (s2 `compose` s1 `compose` s, ())
              ) (nullSubst, ()) e
