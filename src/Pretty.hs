...
instance Pretty Lit where
  ...
  ppr p (LArray expr) = text "[" <> pprArray expr <> text "]" where
    pprArray [] = text ""
    pprArray (x:[]) = ppr p x
    pprArray (x:xs) = ppr p x <> text "," <> pprArray xs 

-- TODO-2: Add the Pretty Pattern instance
instance Pretty Pattern where
  ppr :: Int -> Pattern -> Doc
  ppr _ (PVar var) = text var
  ppr p (PLit lit) = ppr p lit
  ppr p (PCons x xs) = parensIf (p>0) $ ppr (p+1) x <+> text ":" <+> ppr p xs
